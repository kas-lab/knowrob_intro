...
pose_ref_tables = {} # dictionary to store table: pose to store all refrigerated table poses
product_ref_tables = {} # dictionary to store table:product to store all products in refrigerated

for refrig_table in refrig_tables: # go through all refrigerated tables
    # get pose
    pose_refrig = table_pose(refrig_table)
    pose_ref_tables[refrig_table] = pose_refrig
    # get products
    products_ref = products_in_table(refrig_table)
    product_ref_tables[refrig_table] = products_ref
...
pose_tables = {} # dictionary to store table: pose to store all non-refrigerated table poses
product_tables = {} # dictionary to store table:product to store all products in non-refrigerated tables

for table in tables: # go through all tables
    # get pose
    pose_tb = table_pose(table)
    pose_tables[table] = pose_tb
    # get products
    products_no_ref = products_in_table(table)
    product_tables[table] = products_no_ref
...
for product_in_table in product_ref_tables.values(): # the values of the dictionary stores the table
    for product in product_in_table:
...
for product_in_table in product_tables.values(): # the values of the dictionary stores the table
    for product in product_in_table:
        string_query = "instance_of(pap:'" + str(product) +"', pap:'Perishable')."
        query = pq.prolog_query(string_query)
        print(("{} is Perishable: ").format(product))
